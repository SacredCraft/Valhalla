...
import { useBrowserPersistStore, useBrowserStore } from "./browser-page";

export function SelectedCopyCut() {
  const { rowSelection, data, setRowSelection } = useBrowserStore(
    (state) => state,
  );

  const { setCutFiles, setCopyFiles } = useBrowserPersistStore();
